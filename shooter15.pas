...
*** without memory holes; tested with: fpc -Criot -gl -gh shooter15.pas
***************************************************************************}
...
procedure drawLogo;
VAR r, dest : TSDL_FRect;
begin
  r.x := 0;
  r.y := 0;
  SDL_GetTextureSize(SDL2Texture, @dest.w, @dest.h);
  r.w := TRUNC(dest.w);
  r.h := TRUNC(dest.h);
  r.h := MIN(reveal, r.h);
  blitRect(SDL2Texture, @r, (SCREEN_WIDTH DIV 2) - (r.w / 2), 100);
  SDL_GetTextureSize(shooterTexture, @dest.w, @dest.h);
  r.w := TRUNC(dest.w);
  r.h := TRUNC(dest.h);
  r.h := MIN(reveal, r.h);
  blitRect(shooterTexture, @r, (SCREEN_WIDTH DIV 2) - (r.w / 2), 250);
end;
...
begin
  ...
  SDL_GetTextureSize(SDL2Texture, @r.w, @r.h);
  reveal_max := TRUNC(r.h);
  reveal := 0;
  timeout := FPS * 5;
end;
