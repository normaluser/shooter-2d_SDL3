...
***************************************************************************
*** Sound and music
*** without memory holes; tested with: fpc -Criot -gl -gh shooter10.pas
***************************************************************************}

PROGRAM Shooter10;
{$mode FPC} {$H+}    { "$H+" necessary for conversion of String to PChar !!; H+ => AnsiString }
{$COPERATORS OFF}
USES SDL3, SDL3_Image, SDL3_Mixer, Math;
...
begin
  ...
  audiofname[4] := 'music/Mercury.ogg';
  ...
  MIX_PlayTrack(S_Mix[4]^.track, option);
  Mix_SetTrackGain(S_Mix[4]^.track, 1.0);                            // Music volume
end;
...
begin
  ...
  app.Window := SDL_CreateWindow('Shooter 10', SCREEN_WIDTH, SCREEN_HEIGHT, windowFlags);
